...
public class PropertiesSetter {
	...
	public void Set(HashMap<String,String> PropertiesValues) throws NoSuchFieldException, SecurityException, IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException {
...
if(SetMutation(value,PropertiesValues)==false)
	...
	boolean SetAlgorithm(String value) throws InstantiationException, IllegalAccessException {
        ...
        Set<Class<? extends OptimizationAlgorithm >> modules = 
        	    reflections.getSubTypesOf(Optimizer.Algorithms.OptimizationAlgorithm.class);
	...
	boolean SetField(String key, String value ) throws NoSuchFieldException, SecurityException, NumberFormatException, IllegalArgumentException, IllegalAccessException {
	...
	boolean SetCrossover(String value,HashMap<String,String> PropertiesValues) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		
		Reflections reflections = new Reflections("org.uma.jmetal.operator.impl.crossover");
		Set<Class<? extends CrossoverOperator >> allClasses = 
			     reflections.getSubTypesOf(CrossoverOperator.class);
		 for (Iterator<Class<? extends CrossoverOperator>> it = allClasses.iterator(); it.hasNext(); ) {
		     ...
		     if(value.contains(operator.getSimpleName())) {
	        	 if(operator.getGenericInterfaces()[0].getTypeName().contains("IntegerSolution") || operator.getGenericInterfaces()[0].getTypeName().contains("PermutationSolution")) {
	        		 ...
	        		 for(Constructor<?> con : Constructors) {
	        			 if(con.getParameterCount()==NumberOfSetter)
	    	        		 AlgorithmParameters.Crossover=(CrossoverOperator<IntegerSolution>) con.newInstance(ConstructorParameters);
	        		 }
	...
	boolean SetMutation(String value,HashMap<String,String> PropertiesValues) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
		
		Reflections reflections = new Reflections("org.uma.jmetal.operator.impl.mutation");
		Set<Class<? extends MutationOperator >> allClasses = 
			     reflections.getSubTypesOf(MutationOperator.class);
	        		 ...
	        		 for(Constructor<?> con : Constructors) {
	        			...
	        			for(Parameter P : ConParameters) {
	        			 ...
	        			 if(con.getParameterCount()==NumberOfSetter && IsConstructorParametersDouble==true)
	    	        		 AlgorithmParameters.Mutation=(MutationOperator<IntegerSolution>) con.newInstance(ConstructorParameters);
	        		 }
	...
	boolean SetConstructorParameters(Method[] methods ,HashMap<String,String> PropertiesValues, Operator operator) throws NumberFormatException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
	...
	boolean SetSelection(String value) throws InstantiationException, IllegalAccessException {
		...
		Set<Class<? extends SelectionOperator >> allClasses = 
			     reflections.getSubTypesOf(SelectionOperator.class);

...
public class MultiAlgorithmsRunnerMultiThreded implements Runnable {
	...
	public void run() {
		...
		if (algorithm != null) {
			...
			try {
				...
				if(algorithm.getMonitor().evaluations==0){// in the case of the Algorithm cannot be monitored we save the  population
	...
	public void Run() throws ParserConfigurationException, TransformerException, SAXException, IOException,
		...
		AlgorithmParameters.MaxEvaluations = AlgorithmParameters.PopulationSize * 2;// multiply two because solution creation is counted one evaluation
		...
		for (; i < iteration; ++i) {
			...
			if (NextPopulation != null) {// null when all iterations have completed
				...
				if(RestPopulation.size()>= RemainNextPopulationToFill)// In the case of a single population algorithm, which only produces one solution when it runs on its own. 
				NextPopulation
						.addAll(RestPopulation.subList(0, RemainNextPopulationToFill));
			...
			//new XML().iterationToXML(iteration-1, i, IterationFile);//  iteration-1 to stop when reach the last iteration. For example, iterations is 5 and the last iteration (i) is 4 that will make to do an extra iteration when run it again   
			new XML().iterationToXML(iteration, i, IterationFile);//  iteration-1 to stop when reach the last iteration. For example, iterations is 5 and the last iteration (i) is 4 that will make to do an extra iteration when run it again   
		...
		// Clean files
/*
		new File(BestPopulationFile).delete();
		new File(NextPopulationFile).delete();
		new File(IterationFile).delete();
		new File(AlgorithmParameters.BestParametersReportEncoded).delete();
